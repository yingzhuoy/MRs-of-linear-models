...
class LinearMRs():
	...
	def MR4(self):
		...
		for i in range(self.itr_cnt):
			...
			X_train_f = np.column_stack((X_train,np.ones((X_train.shape[0],1))))
	...
	def MR6(self):
		...
		for i in range(self.itr_cnt):
			...
			if self.test_program.__name__=='sig_classification':
				X_train_f = np.row_stack((X_train,X_train[np.where(y_train == 0)]))
				y_train_f = np.append(y_train, y_train[np.where(y_train == 0)])
			else:
				X_train_f = np.row_stack((X_train,X_train[np.where(y_train == -1)]))
				y_train_f = np.append(y_train, y_train[np.where(y_train == -1)])
			X_test_f = X_test
			y_test_f = y_test
	...
	def MR8(self):
		...
		for i in range(self.itr_cnt):
			...
			if self.test_program.__name__=='sig_classification':
				...
				temp_index_n = np.array(np.where(np.array(pred)[temp_n] != 0)).flatten()
				temp_index_p = np.array(np.where(np.array(pred)[temp_p] != 1)).flatten()
			else:
				temp_n = np.array(np.where(y_test == -1)).flatten()
				temp_p = np.array(np.where(y_test == 1)).flatten()
				temp_index_n = np.array(np.where(np.array(pred)[temp_n] != -1)).flatten()
				temp_index_p = np.array(np.where(np.array(pred)[temp_p] != 1)).flatten()

			w = mat(w).T
			distance = abs(X_test*w + b)/np.linalg.norm(w)

			temp_disdance_n = np.array(distance[temp_n[temp_index_n]]).flatten()
			temp_distance_p = np.array(distance[temp_p[temp_index_p]]).flatten()
