...
class LogRes():
	def fit(self,dataMatIn, classLabels):
		...
		n=n+1
		alpha = 1e-3														#移动步长,也就是学习速率,控制更新的幅度。
		maxCycles = 30000														#最大迭代次数
		weights = np.ones((n,1)).astype('float64')
		# weights = np.zeros((n,1))
		for k in range(maxCycles):
			h = sigmoid(dataMatrix * weights)								#梯度上升矢量化公式
			error = labelMat - h
			weights = weights + alpha * (dataMatrix.transpose() * error - 10*weights)
			if np.linalg.norm(dataMatrix.transpose() * error - 10*weights) < 1e-6:
				# print("convergence")
